...
# Figure filename
filename_out = '../../../images/sect3/fig6_spectrum_scan_delta.pdf'
# filename_out = '../../../images/svg/fig6_spectrum_scan_delta.svg'
...
def read_data(xi_in):
    ...
    filename_parameters = "./data_files/scan_{}/g1_parameters.txt".format(xi_in[0])
    ...
    for xi_directory in xi_in:
        # Data filenames
        filename_data_r = "./data_files/scan_{}/g1_corr_real.txt".format(xi_directory)
        filename_data_i = "./data_files/scan_{}/g1_corr_imag.txt".format(xi_directory)
...
# Cycle through plots and plot (lol)
for i in range(len(xi_directory)):
    ...
    #--------------#
    # Plot contourf
    contour_plot = ax[i].pcolormesh(X, Y, spectrum_scans[i], shading='auto',
    ...
    #--------------------#
    # Colorbar
    cbar = plt.colorbar(contour_plot, ax=ax[i])
    ...
    #     Axis ticks     #
    #--------------------#
    ax[i].set_xticks(np.arange(-20.0, 30.0, 10.0))
    ax[i].set_xticks(np.arange(-15.0, 30.0, 10.0), minor=True)

    ax[i].set_yticks(np.arange(-120, 160, 60))
    ax[i].set_yticks(np.arange(-90, 160, 60), minor=True)

    #---------------------#
    #     Axis Limits     #
    #---------------------#
    ax[i].set_ylim(-120, 120)
    ax[i].set_xlim(-20.0, 20.0)

    #---------------------#
    #     Axis labels     #
    #---------------------#
    ax[i].set_ylabel(r'$\left( \omega - \omega_{d} \right) / \Gamma$')

ax[2].set_xlabel(r'$\delta / \Gamma$')
#-----------------------#
#     Figure Labels     #
#-----------------------#
xpos = -28
