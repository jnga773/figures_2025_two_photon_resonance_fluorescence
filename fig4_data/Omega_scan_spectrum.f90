...
REAL(KIND=8), DIMENSION(:), ALLOCATABLE                :: Omega_array
! Scan variable
REAL(KIND=8)                                           :: Omega_scan
! Run counter
INTEGER                                                :: run_counter
...
DO index = 0, number_of_scans
  Omega_array(index) = scan_start + DBLE(index) * scan_step
END DO
...
DO index = 0, number_of_scans
  WRITE(1, "(A15, F25.15)") "Omega =", Omega_array(index)
END DO
...
! Set OMP clauses
!$OMP PARALLEL DO PRIVATE(index, Omega_scan, g1_array) SHARED(run_counter)

! Cycle through halfwidth values
DO index = 0, number_of_scans
  ! Grab Omega value
  Omega_scan = Omega_array(index)
